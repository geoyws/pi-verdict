...
## 修正 2026-09-06(Amendment 2026-09-06)

status: accepted · date: 2026-09-06 · 修正对象:定稿 D 第 4 条「无 UI(headless)…取安全侧同副本处置」与后果第 3 条。

**新处置**:headless(无 UI)会话中检测到 `config/pi-verdict.json` 变更时,**只 fail-closed,不回写**——文件原样留在盘上,通知明说「LEFT IN PLACE」并指引重启会话使新配置生效。其余处置不变:

- 扩展副本变更(有无 UI 皆然)→ 一律从快照还原 + fail-closed;同批次里的 config 未经用户表态,不回写;
- 有 UI 的 config 变更 → 仍是 `ctx.ui.select` 双选;Accept 重建基线,Decline/关闭对话框 → 还原 config + fail-closed(**用户自己要求的回滚**,是唯一会回写 config 的路径);
- 删除同属「同类变更」:headless 下被删的 config 不代用户重建,被删的扩展副本仍重建。

**理由**:定稿 D 把「无人可问」当作「取安全侧 = 照副本处置」,但 config 与副本的所有权不同——config 是用户自己的文件,门禁对它的立场本是「仅用户可改」。回写因此不是安全侧,而是代用户否决其刚落地的改动:

- 实测两次事故。本机 live config 是符号链接进 dotfiles 检出:2026-09-05 提交 `b9e61a2` 的 `omp/pi-verdict.json` 改动被某个长驻 headless 会话回写掉,只能以 `d9ffe9b` 重做;2026-09-06 约 10:16 MYT 提交 `fe8ad3a` 同样被回写。回写还顺带把那个 git 检出弄脏(工作区凭空出现「反向 diff」),污染面越出了门禁自身。
- fail-closed 已经守住了本会话的完整性:被改的配置在本会话内一律不生效,且此后每次裁决恒 deny。回写只多做一件事——毁掉用户的编辑;它并不减少任何攻击面。
- 攻击面评估:注入若改了 config,本会话恒 deny(与回写等价),下一会话该配置会被读取——但那与「用户手工编辑后重启」不可区分,本就是 ADR-0001 二期(跨会话基线)要解决的盲区,不该由「撤销用户改动」来近似。

**代价**:headless 会话不再自动清除被篡改的 config;通知与 block reason 因此必须点名文件并说清「未回写、重启生效」(已实现:reason 逐一交代 `restored …` / `left in place …`)。跨会话基线(二期)仍是这条盲区的正解。
